...
async def modules(call: CallbackQuery):
    ...
    await message.edit_text("<b>🧮Калькулятор позволяет выполнять даже сложные математические выражения прямо в сообщении! ✨</b>\n\nОтправьте в сообщении пример и в конце добавьте = и Джарвис решит его!", reply_markup=IMarkup(inline_keyboard=inline_kb_list))
...
async def modules(call: CallbackQuery):
    ...
    inline_kb_list.append([IButton(text="🌐Подробнее", url="https://vubni.gitbook.io/jarvis/besplatnaya-versiya/obychnye-komandy/kalkulyator")])
    inline_kb_list.append([IButton(text="«", callback_data="commands")])
    await message.edit_text("<b>🧮Калькулятор позволяет выполнять даже сложные математические выражения прямо в сообщении! ✨</b>\n\nОтправьте в сообщении пример и в конце добавьте = и Джарвис решит его!", reply_markup=IMarkup(inline_keyboard=inline_kb_list))