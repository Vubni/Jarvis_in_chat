...
async def modules(call: CallbackQuery):
    ...
    await message.edit_text("<b>💬Системные команды позволяют выполнять системные функции Telegram. Такие как выключить уведомления, заблокировать и т.п.! ✨</b>", reply_markup=IMarkup(inline_keyboard=inline_kb_list))
